...
gulp.task("opera:nex", () => {
  return pipe("./tmp/opera/**/*", $.zip("opera.nex"), "./dist");
});
...
function buildJs(prefix = ".", ctx = {}) {
  const ocbetreePrefix = `${prefix}/src/ocbetree`;
  const ocbetreeJsFiles = fs
    .readdirSync(ocbetreePrefix)
    .filter((v) => v.endsWith(".js"))
    .map((v) => `${ocbetreePrefix}/${v}`);
